...
case "$MODE" in
  ...
  policy) PROMPT="执行今日政策变化扫描: 用 global-policy-search 技能, 按 config/sources.yaml 的 policy_queries 检索 WTO ePing 与外国官方检疫机构的动植物检疫管控政策变化(不采集中国海关总署政策), 抽取为政策记录并完成核验与对华影响研判, 最后用 scripts/report.py --excel --docx 生成政策日报并用 scripts/push_report.py 推送到已配置渠道" ;;
